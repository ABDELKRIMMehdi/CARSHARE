<?php
 ...
 if(isset($_POST['function'])) {
     if($_POST['function'] == "deleteUser") {
        ...
        mysqli_stmt_bind_param($stmt2, "i", $_POST['id']);
        ...
        if(mysqli_num_rows($resultData) > 0){
            while($row = mysqli_fetch_assoc($resultData)){
                $sql3 = "SELECT * FROM participants WHERE rideId = ?;";
        ...
        if(mysqli_num_rows($resultData2) > 0){
            while($row2 = mysqli_fetch_assoc($resultData2)){
                ...
                $date = date('d-m-y');
                $read = "no";
                $content = "<div class='cancelride'><span id='start'> The ride going from : " . $row['startingPoint'] . " </sapn><span id='arrival'> to : " . $row['startingPoint'] . " </span><span id='date'> on the : " . $row['journeyDate'] . " </span><span id='heure'> at : " . $row['hour']  . " </span><p id='para1'> HAS BEEN CANCELED</p><span class = 'daysent'>". $date ."</span></div>";
...
/* end notification */
        ...
        mysqli_stmt_bind_param($stmt5, "i", $_POST['id']);
            ...
            while($row5 = mysqli_fetch_assoc($resultData5)){
                ...
                if(mysqli_num_rows($resultData6) > 0){
                    while($row6 = mysqli_fetch_assoc($resultData6)){
                        ...
                        $sql8 = "UPDATE rides SET numLeft = numLeft + 1 where rideId = ?;";
